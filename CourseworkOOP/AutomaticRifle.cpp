...
AutomaticRifle::AutomaticRifle(std::string name, std::string description, const Ammo ammunition, int magazine_capacity, int amount_of_ammo) : Automatic(name, description, ammunition, magazine_capacity, amount_of_ammo)
{
}

std::string AutomaticRifle::ShootWholeMag()
{
	if (weapon_health > 0 && magazine.size() > 0)
	{
		...
		for (int i = 0; i < size - 1; i++)
		{
			...
			std::cout << "Boom ";
			weapon_health -= ammunition.GetDamageToWeapon() * 4;
			magazine.pop();
		}
		total_damage += ammunition.GetDamage();
